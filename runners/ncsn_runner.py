...
class NCSNRunner():
    ...
    def sample_general(self, score, samples, init_samples, sigma_0, sigmas, num_variations = 8, deg = 'sr4'):
        ...
        if deg[:2] == 'cs':
            ...
            # H=H[:, (self.config.data.image_size * 20): (self.config.data.image_size * 40)]
            # H = H.T
        elif deg == 'inp_v2':
            ## crop ##
            H= torch.ones(sample_gt.shape[2],sample_gt.shape[3]).to(self.config.device)
            H[:,20:40] = 0
        elif deg == 'deblur_uni':
            ## blur ##
        ...
        if deg=='inp_v2':
            y_0 = samples + sigma_0 * torch.randn_like(samples)
            y_0 = y_0*H
        ...
        # if deg == 'syn': pinv_y_0 = stochastic_variations[0: self.config.sampling.batch_size, :, :, :]
        ...
        for i in range(num_variations):
            ...
            all_samples, x_t_list = general_anneal_Langevin_dynamics_den(H, y_0, init_samples, score, sigmas,
                                                           self.config.sampling.n_steps_each,
                                                           self.config.sampling.step_lr, verbose=True,
                                                           final_only=self.config.sampling.final_only,
            ...
            stochastic_variations_x_t = torch.zeros((2 + len(x_t_list)*num_variations) * self.config.sampling.batch_size,
