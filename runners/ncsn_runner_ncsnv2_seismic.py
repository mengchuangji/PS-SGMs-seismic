        ...
        if not self.config.sampling.fid:
            if self.config.sampling.inpainting:
                ...
                if not self.config.sampling.final_only:
                    ...
                    plt.imshow(image_grid.numpy().squeeze().transpose((1, 2, 0))[:, :, 0], cmap=plt.cm.seismic, vmin=-1,
                               vmax=1)
                    # plt.colorbar()  # 添加色标
                    plt.axis('off')  # 关闭坐标轴
                    plt.tight_layout()
                    plt.savefig(os.path.join(os.path.join(self.args.image_folder,
                                                          'image_grid_{}_inter_1.png'.format(self.config.sampling.ckpt_id))),
                                format='jpg',
                                dpi=300,bbox_inches='tight')
                    torch.save(sample, os.path.join(self.args.image_folder,
                                                    'samples_{}_inter_5.pth'.format(self.config.sampling.ckpt_id)))
                ...
                if self.config.sampling.data_init:
                    ...
                    init_samples = torch.rand(self.config.sampling.batch_size, self.config.data.channels,
                                              ...
                                              device=self.config.device)
                    init_samples = data_transform(self.config, init_samples)
                torch.manual_seed(20240313)
                all_samples , x_t_list= anneal_Langevin_dynamics(init_samples, score, sigmas,
                                                       self.config.sampling.n_steps_each,
                                                       self.config.sampling.step_lr, verbose=False,
                                                       final_only=self.config.sampling.final_only,
                                                       denoise=self.config.sampling.denoise)
                ...
                if not self.config.sampling.final_only:
                    ...
                    sample = inverse_data_transform(self.config, sample)
                    import math
                    image_grid = make_grid(sample, nrow=int(np.sqrt(self.config.sampling.batch_size)), padding=4)
                    # save_image(image_grid, os.path.join(self.args.image_folder,
                    #                                     'image_grid_{}.png'.format(self.config.sampling.ckpt_id)))
                    import matplotlib.pyplot as plt
                    plt.gcf().set_size_inches(15, 15)
                    ...
                    plt.tight_layout()
                    plt.savefig(os.path.join(os.path.join(self.args.image_folder,
                                                          'image_grid_{}_gener_240313.png'.format(
                                                              self.config.sampling.ckpt_id))),
                                format='jpg',
                                dpi=300, bbox_inches='tight')
                    torch.save(sample, os.path.join(self.args.image_folder,
                                                    'samples_{}_gener_240313.pth'.format(self.config.sampling.ckpt_id)))

                    stochastic_variations_x_t = torch.zeros(
                        len(x_t_list) * self.config.sampling.batch_size,
                    ...
                    for x_t in x_t_list:
                        ...
                        (self.config.sampling.batch_size) * i: (self.config.sampling.batch_size) * (i + 1), :, :,
                    ...
                    # plt.colorbar()  # 添加色标
                    plt.axis('off')  # 关闭坐标轴
                    plt.savefig(os.path.join(self.args.image_folder, 'stochastic_variation_x_t.png'), format='jpg', dpi=300,
                                bbox_inches='tight')
