...
def get_custom_kernel(type = "gauss", dim = 64):
    ...
    # elif type == "ricker":
    #     from utils.covolution_model import ricker
    #     dt = 0.001  # 时间采样间隔/s
    #     fn = 35  #
    #     kernel, _ = ricker(fn, dt)
    #     L = len(kernel) // 2
    #     kernel=kernel.reshape(len(kernel),-1)
    #     aa=np.r_[np.zeros(L, dtype=int),1, np.zeros(L, dtype=int)].reshape(-1,len(kernel))
    #     aa = np.ones_like(kernel).reshape(-1, len(kernel))
    #     kernel = np.matmul(kernel,aa).T/len(aa)
